"""Schema version helpers and migration registry for Checkpoint Manager."""
...
def migrate_payload(payload: Mapping[str, Any]) -> dict[str, Any]:
    """Migrate a serialized checkpoint payload to SCHEMA_VERSION."""
    data = dict(payload)
    ...
    while version < SCHEMA_VERSION:
        ...
        if migrator is None:
            # Identity path reserved until SCHEMA_VERSION bumps
            break
        data = migrator(data)
        version += 1
