...
class ExecutionPipeline:
    ...
    def _to_response(
        ...
        self.runtime.metrics.record_inference(
            ...
            prompt_tokens=result.prompt_tokens,
            completion_tokens=result.completion_tokens,
            llama_prompt_ms=_metric_float(result.metrics, "llama_prompt_ms"),
            llama_predicted_ms=_metric_float(result.metrics, "llama_predicted_ms"),
            llama_prompt_per_second=_metric_float(result.metrics, "llama_prompt_per_second"),
            llama_predicted_per_second=_metric_float(result.metrics, "llama_predicted_per_second"),
        )
        # Mirror server decode tok/s onto nexus_* for Grafana / RMF.
        pred = _metric_float(result.metrics, "llama_predicted_per_second")
        if pred is not None and pred > 0:
            try:
                bridge = getattr(self.runtime.metrics, "bridge", None)
                if bridge is not None and hasattr(bridge, "set"):
                    bridge.set("nexus_tokens_per_second", float(pred))
            except Exception:
                pass
