...
class DraftModelProposer(ProposalStrategy):
    ...
    def __init__(self, backend_name: str = "tier1") -> None:
        ...
        self._draft_quant: str | None = None
        self._host_arch: str | None = None
    ...
    async def initialize(self, ctx: ASCRInitContext) -> None:
        ...
        self._resolve_draft_identity(cfg, strategies)

    def _resolve_draft_identity(
        self, cfg: dict[str, Any], strategies: dict[str, Any]
    ) -> None:
        """NSA_DRAFT_MODEL_PATH → registry resolve → tier1 fallback."""
        from neuroswarm_arm.runtime.armcascade.proposal.draft_registry import (
            DraftModelRegistry,
            detect_host_arch,
        )

        draft_reg_cfg = dict(cfg.get("draft_registry") or {})
        strat_reg = dict(strategies.get("draft_registry") or {})
        # strategies.draft_registry wins for enable; merge pairs from both
        enabled = bool(strat_reg.get("enabled", draft_reg_cfg.get("enabled", False)))
        merged_pairs_cfg: dict[str, Any] = {**draft_reg_cfg, **strat_reg}

        env_path = (os.getenv("NSA_DRAFT_MODEL_PATH") or "").strip()
        env_quant = (os.getenv("NSA_DRAFT_QUANT") or "Q4_0").strip() or "Q4_0"
        auto = _env_bool("NSA_DRAFT_REGISTRY_AUTO", enabled)

        self._host_arch = detect_host_arch()
        self._target_model = _resolve_target_model(cfg)
        self._draft_path = None
        self._draft_quant = None

        if env_path:
            self._draft_path = env_path
            self._draft_quant = env_quant
            return

        if not enabled and not auto:
            return
        if not auto:
            return

        reg = DraftModelRegistry(merged_pairs_cfg)
        hit = reg.resolve(self._target_model or "", self._host_arch or "x86")
        if hit is None:
            return
        self._draft_path, self._draft_quant = hit
    ...
    async def propose(self, req: ProposalRequest) -> Proposal:
        ...
        if self._draft_path:
            meta["draft_path"] = self._draft_path
            meta["draft_quant"] = self._draft_quant or "Q4_0"
            meta["host_arch"] = self._host_arch
            meta["target_model"] = self._target_model
...
def _resolve_target_model(cfg: dict[str, Any]) -> str | None:
    env = (os.getenv("NSA_TARGET_MODEL") or "").strip()
    if env:
        return env
    for t in cfg.get("tiers") or []:
