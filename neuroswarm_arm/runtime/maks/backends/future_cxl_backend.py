"""Future CXL provider — interface only (no public CXL pools on Axion today)."""
...
class FutureCXLBackend(BaseKVProvider):
    """Placeholder for CXL memory-pool tiering."""
    ...
    def _raise(self) -> None:
        raise KVProviderUnavailableError(
            "CXL memory pools not publicly available on GCP Axion; use nvme/mmap cold tiers"
        )
    ...
    async def stats(self) -> ProviderStats:
        return ProviderStats(name=self.name, available=False, extra={"reason": "cxl_unavailable"})
